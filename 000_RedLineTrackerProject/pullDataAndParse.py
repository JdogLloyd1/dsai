...
#
# MBTA API assumptions (per https://api-v3.mbta.com/docs/swagger/index.html and GTFS):
# - Prediction/Schedule: attributes departure_time, arrival_time (ISO 8601); schedule has same.
# - Red Line direction_id: 0 = southbound (Braintree/Ashmont), 1 = northbound (Alewife).
# - At Alewife: departures = direction_id 0 (southbound); arrivals = direction_id 1 (northbound).
# - Alerts: attributes severity (1=Information, 2=Warning, 3=Emergency), active_period [{start,end}].
# - Vehicle: relationships stop; included stop has attributes.name for current stop.
...
for item in pred_payload.get("data", []):
    ...
    # Schedule may use departure_time/arrival_time or alternate keys; support both
    ...
    # #region agent log
    if _pred_idx < 5:
        _log("pullDataAndParse.py:pred_loop", "per-prediction", {"idx": _pred_idx, "direction_id": direction_id, "direction_id_eq_0": direction_id == 0, "direction_id_eq_1": direction_id == 1, "pred_dep": pred_dep is not None, "pred_arr": pred_arr is not None, "sched_attrs_keys": list(sched_attrs.keys()) if sched_attrs else [], "sched_dep_raw": sched_dep, "sched_arr_raw": sched_arr, "schedule_id": schedule_id, "trip_id": trip_id, "schedule_in_lookup": (schedule_id and ("schedule", schedule_id) in included), "trip_in_lookup": (trip_id and ("trip", trip_id) in included)}, "H-A,H-B,H-C")
    _pred_idx += 1
    # #endregion

    # Departures from Alewife: southbound = direction_id 0 (per MBTA/GTFS)
    _dep_branch = direction_id == 0 and (pred_dep or sched_dep or pred_arr)
    if _dep_branch:
        status = derive_status(sched_dep_dt, pred_dep_dt or pred_arr_dt)
    ...
    # Arrivals to Alewife: northbound = direction_id 1 (per MBTA/GTFS)
    _arr_branch = direction_id == 1 and (pred_arr or sched_arr or pred_dep)
    if _arr_branch:
        status = derive_status(sched_arr_dt, pred_arr_dt or pred_dep_dt)
...
# #region agent log
_log("pullDataAndParse.py:after_loop", "departures/arrivals counts", {"departures_rows": len(departures_rows), "arrivals_rows": len(arrivals_rows), "runId": "post-fix"}, "H-E")
# #endregion
...
# #region agent log
_arr_dt_sample = list(arrival_dt.head(3).astype(str)) if len(arrival_dt) else []
_log("pullDataAndParse.py:time_filter", "time window", {"now_utc": str(now), "ten_min_utc": str(ten_min), "sixty_min_utc": str(sixty_min), "arrival_dt_sample": _arr_dt_sample, "mask_10_sum": int(mask_10.sum()), "mask_60_sum": int(mask_60.sum()), "arrivals_raw_len": len(df_arrivals_raw)}, "H-D,H-E")
# #endregion
