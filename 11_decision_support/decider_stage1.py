# decider_stage1.py
# Stage 1 Wedding Venue Decider (Ollama Cloud)
# Pairs with ACTIVITY_decider.md
# Jonathan Lloyd
#
# This script sends the Stage 1 wedding venue task to Ollama Cloud and
# asks for a strict, structured decision output. It saves both the model's
# markdown response and the raw JSON API response for transparency.
...
# Keep key runtime settings explicit in script-level constants.
...
# Read repo-level .env from project root. We do not create or edit this file.
...
## 0.4 Stage 1 prompts and venue data ############################
...
and recommend the top 3 venues based on the client's priorities.
...
def build_payload() -> dict:
    """Build the Ollama chat payload for Stage 1."""
    return {
        "model": OLLAMA_MODEL,
        "messages": [
...
def validate_response(markdown_text: str) -> list[str]:
    """Run lightweight checks for the required formatting contract."""
    issues = []
...
print("\n" + "=" * 62)
print("DECIDER STAGE 1: OLLAMA CLOUD REQUEST")
print("=" * 62)
